...
1. For the project creation test, we will use the `faker` library, which will help us with the creation of random data. In the terminal, on the project root path, run the following command `npm i faker@5.5.3 -D` (this command will install the `faker` library as a dev dependency)
